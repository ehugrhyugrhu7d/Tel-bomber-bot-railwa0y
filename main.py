...
def init_db() -> None:
    with closing(db_connect()) as conn:
        ...
        # Ensure main admin is in admins table
        conn.execute(
...
def user_menu_markup(user_id: int) -> InlineKeyboardMarkup:
    ...
    if not has_sub:
        ...
        if username and not any(b["action"] == "support" for b in buttons):
            rows.append([InlineKeyboardButton("💬 تماس با پشتیبانی", url=f"https://t.me/{username}")])
    return InlineKeyboardMarkup(rows)
...
def support_markup() -> InlineKeyboardMarkup:
    username = get_setting("support_username", SUPPORT_USERNAME).strip().lstrip("@")
    if username:
        return InlineKeyboardMarkup([[InlineKeyboardButton("💬 تماس با پشتیبانی", url=f"https://t.me/{username}")], [back_button("user:menu")]])
...
async def admin_add_button_from_text(update, context, text):
    # Format: scope (active/inactive) | label | action
...
async def admin_edit_button_from_text(update, context, text):
    # Format: id | scope | label | action | enabled
    parts = [p.strip() for p in text.split("|")]
