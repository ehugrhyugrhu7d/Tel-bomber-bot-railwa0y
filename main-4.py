...
DEFAULT_BUTTONS = [
    ("ثبت سفارش 📱", "order", 1),
    ("سفارش‌های من 📦", "my_orders", 2),
    ("حساب کاربری 👤", "account", 3),
    ("پشتیبانی 💬", "support", 4),
]
...
def init_db() -> None:
    with closing(db_connect()) as conn:
        ...
        count = conn.execute("SELECT COUNT(*) FROM buttons WHERE scope='user'").fetchone()[0]
        if count == 0:
            conn.executemany(
                "INSERT INTO buttons(scope,label,action,enabled,sort_order) VALUES('user',?,?,1,?)",
                [(label, action, order) for label, action, order in DEFAULT_BUTTONS],
            )
...
def user_menu_markup() -> InlineKeyboardMarkup:
    rows = []
    buttons = db_fetchall(
        "SELECT id,label,action FROM buttons WHERE scope='user' AND enabled=1 ORDER BY sort_order,id"
    )
    current = []
    for row in buttons:
...
async def cancel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin_user(update):
        ...
        await update.effective_message.reply_text("🏠 منوی اصلی", reply_markup=user_menu_markup())
...
async def admin_add_user_from_text(update, context, text):
    if not re.fullmatch(r"\d+", text):
        await update.effective_message.reply_text("❌ فقط Numeric Telegram ID وارد کن.")
        return
    uid = int(text)
    ensure_user_id(uid)
    context.user_data["target_user_id"] = uid
    context.user_data["state"] = "addsub"
    await update.effective_message.reply_text("✅ کاربر ثبت شد. حالا تعداد روز اشتراک را وارد کن.")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "user:menu":
        ...
        if user_has_subscription(user_id):
            await q.edit_message_text("🏠 منوی اصلی", reply_markup=user_menu_markup())
        else:
    ...
    if data.startswith("user:action:"):
        ...
        if action == "order":
            ...
            await q.edit_message_text(get_text("unknown"), reply_markup=user_menu_markup())
        return
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str) -> None:
    ...
    if data == "admin:adduser":
        ...
        await q.edit_message_text("➕ Numeric Telegram ID کاربر را ارسال کن:", reply_markup=InlineKeyboardMarkup([[btn("❌ لغو", "admin:menu")]]))
        return
