...
DEFAULT_BUTTONS = [
    ("active", "ثبت سفارش 📱", "order", 1),
    ("active", "سفارش‌های من 📦", "my_orders", 2),
    ("active", "حساب کاربری 👤", "account", 3),
...
class MutationResult:
    def __init__(self, rowcount: int, lastrowid: int):
        self.rowcount = rowcount
        self.lastrowid = lastrowid


def db_exec(sql: str, params: tuple = ()) -> MutationResult:
    with closing(db_connect()) as conn:
        cur = conn.execute(sql, params)
        return MutationResult(cur.rowcount, cur.lastrowid)
...
def init_db() -> None:
    with closing(db_connect()) as conn:
        conn.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_pending_user_grants_username ON pending_user_grants(username);
            CREATE INDEX IF NOT EXISTS idx_pending_admin_grants_username ON pending_admin_grants(username);
        ...
        for scope, label, action, order in DEFAULT_BUTTONS:
            conn.execute(
                "INSERT OR IGNORE INTO buttons(scope,label,action,enabled,sort_order) VALUES(?,?,?,?,?)",
                (scope, label, action, 1, order),
            )
        conn.execute(
            "INSERT INTO admins(user_id,username,role,enabled,created_at) VALUES(?,?,?,?,?) "
            "ON CONFLICT(user_id) DO UPDATE SET username=excluded.username, role='owner', enabled=1",
...
def valid_username(value: str) -> bool:
    return bool(re.fullmatch(r"[A-Za-z0-9_]{5,32}", normalize_username(value)))


def is_admin_id(user_id: int) -> bool:
    if int(user_id) == ADMIN_ID:
        return True
    row = db_fetchone("SELECT enabled FROM admins WHERE user_id=?", (int(user_id),))
    return bool(row and row["enabled"])


def apply_pending_grants(user_id: int, username: str) -> None:
    username = normalize_username(username)
    grant = db_fetchone("SELECT * FROM pending_user_grants WHERE username=?", (username,))
    if grant:
        set_subscription(user_id, int(grant["days"]), renew=False)
        db_exec("DELETE FROM pending_user_grants WHERE username=?", (username,))
        log_event(user_id, "pending_subscription_applied", f"days={grant['days']}")


def apply_pending_admin_grant(user_id: int, username: str) -> None:
    username = normalize_username(username)
    grant = db_fetchone("SELECT * FROM pending_admin_grants WHERE username=?", (username,))
    if grant:
        db_exec("INSERT OR REPLACE INTO admins(user_id,username,role,enabled,created_at) VALUES(?,?,?,?,?)", (user_id, username, grant["role"], 1, now_iso()))
        db_exec("DELETE FROM pending_admin_grants WHERE username=?", (username,))
        log_event(user_id, "admin_grant_applied")
...
def ensure_user(tg_user) -> None:
    ...
    with closing(db_connect()) as conn:
        conn.execute(
            ...
            (
                uid,
                normalize_username(tg_user.username or ""),
    ...
    username = normalize_username(tg_user.username or "")
    if username:
        apply_pending_grants(uid, username)
        apply_pending_admin_grant(uid, username)
...
def admin_menu_markup() -> InlineKeyboardMarkup:
    rows=[]; current=[]
    try:
        items=db_fetchall("SELECT label,action FROM buttons WHERE scope='admin' AND enabled=1 ORDER BY sort_order,id")
        for r in items:
            data_map={"users":"admin:users:0","adduser":"admin:adduser","admins":"admin:admins:0","addadmin":"admin:addadmin","subs":"admin:subs:0","orders":"admin:orders:0","tickets":"admin:tickets:0","stats":"admin:stats","logs":"admin:logs:0","buttons":"admin:buttons","texts":"admin:texts","broadcast":"admin:broadcast","backup":"admin:backup","settings":"admin:settings","health":"admin:health"}
            data=data_map.get(r["action"])
            if not data: continue
            current.append(btn(r["label"],data))
            if len(current)==2: rows.append(current); current=[]
        if current: rows.append(current)
    except Exception:
        rows=[]
    if not rows:
        rows=[[btn("👥 کاربران","admin:users:0"),btn("➕ افزودن کاربر","admin:adduser")],[btn("👑 ادمین‌ها","admin:admins:0"),btn("➕ افزودن ادمین","admin:addadmin")],[btn("💳 اشتراک‌ها","admin:subs:0")],[btn("📦 سفارش‌ها","admin:orders:0"),btn("🎫 تیکت‌ها","admin:tickets:0")],[btn("📊 آمار","admin:stats"),btn("🧾 لاگ‌ها","admin:logs:0")],[btn("🔘 دکمه‌ها","admin:buttons"),btn("✏️ متن‌ها","admin:texts")],[btn("📢 Broadcast","admin:broadcast"),btn("💾 Backup","admin:backup")],[btn("⚙️ تنظیمات","admin:settings"),btn("🩺 وضعیت","admin:health")]]
...
def user_menu_markup(user_id: Optional[int] = None) -> InlineKeyboardMarkup:
    rows = []
    uid = user_id
    if uid is None:
        uid = 0
    scope = "active" if user_has_subscription(uid) else "guest"
    buttons = db_fetchall(
        "SELECT id,label,action FROM buttons WHERE scope=? AND enabled=1 ORDER BY sort_order,id", (scope,)
    )
    current = []
    for row in buttons:
    ...
    if current:
        rows.append(current)
    if not rows:
        fallback = [("ثبت سفارش 📱", "order"), ("سفارش‌های من 📦", "my_orders"), ("حساب کاربری 👤", "account"), ("پشتیبانی 💬", "support")] if scope == "active" else [("پشتیبانی 💬", "support")]
        rows = [[btn(label, f"user:action:{action}")] for label, action in fallback]
    return InlineKeyboardMarkup(rows)
...
def is_admin_user(update: Update) -> bool:
    ...
    return bool(user and is_admin_id(int(user.id)))
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if existing is None:
        await update.effective_message.reply_text(
            ...
            reply_markup=user_menu_markup(user.id) if sub else support_markup(),
        )
        return
    if not sub:
        await update.effective_message.reply_text(
            safe_format(get_text("no_subscription"), name=user.first_name or "دوست عزیز"),
...
async def cancel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin_user(update):
        await update.effective_message.reply_text("🛠 پنل مدیریت", reply_markup=admin_menu_markup())
    elif user_has_subscription(update.effective_user.id):
...
async def show_my_tickets(update: Update, page: int = 0) -> None:
    ...
    text="🎫 *تیکت‌های من*\n\n"+"\n".join(f"• #{r['id']} — {r['status']}" for r in rows) if rows else "🎫 هنوز تیکتی ندارید."
    kb=[[btn(f"#{r['id']}",f"user:ticket:{r['id']}")] for r in rows]; nav=[]
    if page>0: nav.append(btn("◀️",f"user:tickets:{page-1}"))
    nav.append(btn(f"{page+1}/{pages}","user:noop"))
    if page<pages-1: nav.append(btn("▶️",f"user:tickets:{page+1}"))
    kb.append(nav); kb.append([back_button("user:menu")]); await edit_or_reply(update,text,InlineKeyboardMarkup(kb),ParseMode.MARKDOWN)
...
async def handle_text_state(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    elif state == "pending_user_days":
        if not re.fullmatch(r"\d{1,5}", text) or int(text) < 1:
            await update.effective_message.reply_text("❌ تعداد روز نامعتبر است.")
            return
        username = context.user_data.get("pending_username")
        db_exec("INSERT INTO pending_user_grants(username,days,created_at,created_by) VALUES(?,?,?,?) ON CONFLICT(username) DO UPDATE SET days=excluded.days,created_at=excluded.created_at,created_by=excluded.created_by", (username,int(text),now_iso(),ADMIN_ID))
        context.user_data.clear(); log_event(ADMIN_ID,"Add User",f"pending_username={username};days={text}")
        await update.effective_message.reply_text(f"✅ Pending Grant برای @{username} با مدت {text} روز ثبت شد.",reply_markup=admin_menu_markup())
...
async def admin_add_user_from_text(update, context, text):
    ...
    if not valid_username(text):
        await update.effective_message.reply_text("❌ Username نامعتبر است. مثال: @Mmdm_mk")
        return
    ...
    if row:
        ...
        await update.effective_message.reply_text("✅ کاربر پیدا شد. تعداد روز اشتراک را وارد کن.")
    else:
        context.user_data["pending_username"] = username
        context.user_data["state"] = "pending_user_days"
        await update.effective_message.reply_text("ℹ️ کاربر هنوز /start نزده. تعداد روز اشتراک را وارد کن تا Pending Grant ثبت شود.")
...
async def admin_subscription_from_text(update, context, text):
    ...
    try:
        await context.bot.send_message(uid, safe_format(get_text(msg_key), expires=human_dt(sub["expires_at"])))
    except TelegramError:
        pass
    await update.effective_message.reply_text(
...
async def admin_add_button_from_text(update, context, text):
    ...
    if len(parts) != 2 or not parts[0] or not parts[1]:
        await update.effective_message.reply_text("❌ فرمت: نام دکمه | action")
        return
    label, action = parts
    with closing(db_connect()) as conn:
        max_order = conn.execute("SELECT COALESCE(MAX(sort_order),0) FROM buttons WHERE scope='user'").fetchone()[0]
        conn.execute("INSERT INTO buttons(scope,label,action,enabled,sort_order) VALUES('user',?,?,1,?)", (label[:100], action[:100], max_order + 1))
    context.user_data.clear()
    log_event(ADMIN_ID, "button_added", f"label={label};action={action}")
...
async def admin_edit_button_from_text(update, context, text):
    # Format: id | label | action | enabled
    parts = [p.strip() for p in text.split("|")]
    if len(parts) != 4 or not parts[0].isdigit() or parts[3] not in {"0", "1"}:
        await update.effective_message.reply_text("❌ فرمت: ID | label | action | enabled(0/1)")
        return
    bid = int(parts[0])
    row = db_fetchone("SELECT id FROM buttons WHERE id=? AND scope='user'", (bid,))
    if not row:
        await update.effective_message.reply_text("❌ دکمه پیدا نشد.")
        return
    db_exec("UPDATE buttons SET label=?,action=?,enabled=? WHERE id=?", (parts[1][:100], parts[2][:100], int(parts[3]), bid))
    context.user_data.clear()
...
async def admin_delete_button_from_text(update, context, text):
    if not text.isdigit():
        ...
        return
    bid = int(text)
    db_exec("DELETE FROM buttons WHERE id=? AND scope='user'", (bid,))
    context.user_data.clear()
...
async def admin_reorder_button_from_text(update, context, text):
    ...
    db_exec("UPDATE buttons SET sort_order=? WHERE id=? AND scope='user'", (order, bid))
    context.user_data.clear()
...
async def admin_search_user_from_text(update, context, text):
    context.user_data.clear()
    if text.isdigit():
        row = db_fetchone("SELECT * FROM users WHERE user_id=?", (int(text),))
    else:
        username = text.lstrip("@").lower()
        row = db_fetchone("SELECT * FROM users WHERE lower(username)=?", (username,))
    if not row:
...
async def admin_users(update: Update, page: int):
    ...
    kb = [[btn(f"{r['user_id']}", f"admin:user:{r['user_id']}")] for r in rows]
    nav = []
...
async def admin_admins(update: Update, page: int = 0):
    size=get_page_size(); total=db_fetchone("SELECT COUNT(*) c FROM admins")['c']; pages=max(1,(total+size-1)//size); page=max(0,min(page,pages-1))
    rows=db_fetchall("SELECT * FROM admins ORDER BY created_at DESC LIMIT ? OFFSET ?",(size,page*size))
    text="👑 *ادمین‌ها*\n\n"+"\n".join(f"• `{r['user_id']}` — @{r['username'] or '-'} — {r['role']} — {'فعال' if r['enabled'] else 'خاموش'}" for r in rows)
    kb=[[btn(f"@{r['username'] or r['user_id']}",f"admin:admintoggle:{r['user_id']}")] for r in rows if int(r['user_id'])!=ADMIN_ID]
    nav=[]
    if page>0: nav.append(btn("◀️",f"admin:admins:{page-1}"))
    nav.append(btn(f"{page+1}/{pages}","admin:noop"))
    if page<pages-1: nav.append(btn("▶️",f"admin:admins:{page+1}"))
    kb.append(nav); kb.append([btn("➕ افزودن ادمین","admin:addadmin"),back_button("admin:menu")])
    await edit_or_reply(update,text or "👑 ادمینی وجود ندارد.",InlineKeyboardMarkup(kb),ParseMode.MARKDOWN)


async def admin_add_admin_from_text(update, context, text):
    username=normalize_username(text)
    if not valid_username(text):
        await update.effective_message.reply_text("❌ Username نامعتبر است. مثال: @Mmdm_mk"); return
    row=db_fetchone("SELECT user_id FROM users WHERE lower(username)=?",(username,))
    if row:
        db_exec("INSERT OR REPLACE INTO admins(user_id,username,role,enabled,created_at) VALUES(?,?,?,?,?)",(row['user_id'],username,'admin',1,now_iso()))
        context.user_data.clear(); log_event(ADMIN_ID,"Add Admin",f"user_id={row['user_id']};username={username}")
        await update.effective_message.reply_text(f"✅ @{username} ادمین شد.",reply_markup=admin_menu_markup())
    else:
        ...
        context.user_data.clear(); log_event(ADMIN_ID,"Add Admin",f"pending_username={username}")
        await update.effective_message.reply_text(f"✅ Pending Admin برای @{username} ثبت شد.",reply_markup=admin_menu_markup())
...
async def admin_buttons(update: Update):
    rows = db_fetchall("SELECT * FROM buttons WHERE scope='user' ORDER BY sort_order,id")
    text = "🔘 *دکمه‌های کاربران*\n\n" + "\n".join(
        f"• ID {r['id']} | {r['label']} | `{r['action']}` | {'فعال' if r['enabled'] else 'خاموش'} | sort={r['sort_order']}" for r in rows
    )
    kb = [
...
def validate_backup_bytes(data: bytes) -> tuple[bool, str]:
    ...
    try:
        ...
        required = {"users", "subscriptions", "orders", "tickets", "ticket_messages", "logs", "buttons", "texts", "settings"}
    ...
    try:
        ...
        for sidecar in (old_wal, old_shm):
            try:
                ...
                pass
        # Set rollback journal mode in the incoming DB so stale WAL files cannot affect it.
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Any callback from a blocked normal user is rejected.
    ...
    if data == "user:menu":
        ...
        if user_has_subscription(user_id):
            await q.edit_message_text("🏠 منوی اصلی", reply_markup=user_menu_markup(user_id))
        else:
            await q.edit_message_text(get_text("no_subscription"), reply_markup=support_markup())
        return
    ...
    if data.startswith("user:action:"):
        ...
        if action == "order":
            ...
            await show_my_tickets(update, 0)
        else:
    ...
    if data.startswith("user:ticket:"):
        tid=data.split(":",2)[2]
        row=db_fetchone("SELECT * FROM tickets WHERE id=? AND user_id=?",(tid,user_id))
        if not row:
            await q.edit_message_text("❌ تیکت پیدا نشد.",reply_markup=InlineKeyboardMarkup([[back_button("user:tickets:0")]])); return
        msgs=db_fetchall("SELECT * FROM ticket_messages WHERE ticket_id=? ORDER BY created_at ASC LIMIT 50",(tid,))
        text=f"🎫 *تیکت #{tid}*\n\n"+"\n\n".join(("شما" if m["sender_role"]=="user" else "پشتیبانی")+f": {m['text']}" for m in msgs)
        await q.edit_message_text(text,parse_mode=ParseMode.MARKDOWN,reply_markup=InlineKeyboardMarkup([[back_button("user:tickets:0")]])); return

    # Admin callbacks: every one re-checks admin identity.
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str) -> None:
    ...
    if data == "admin:stats":
        ...
        return
    if cmd == "admins":
        try: await admin_admins(update, int(parts[2]))
        except (ValueError,IndexError): await q.answer("صفحه نامعتبر",show_alert=True)
        return
    if data == "admin:addadmin":
        context.user_data["state"]="add_admin"
        await q.edit_message_text("➕ Username ادمین جدید را ارسال کن (مثال: @username):")
        return
    if cmd == "admintoggle":
        try: uid=int(parts[2])
        except (ValueError,IndexError): await q.answer("شناسه نامعتبر",show_alert=True); return
        if uid==ADMIN_ID: await q.answer("Owner قابل حذف یا غیرفعال کردن نیست.",show_alert=True); return
        row=db_fetchone("SELECT enabled FROM admins WHERE user_id=?",(uid,))
        if not row: await q.answer("ادمین پیدا نشد.",show_alert=True); return
        new=0 if row['enabled'] else 1; db_exec("UPDATE admins SET enabled=? WHERE user_id=?",(new,uid)); log_event(ADMIN_ID,"Remove Admin" if not new else "Add Admin",f"user_id={uid}")
        await admin_admins(update,0); return
    if data == "admin:stats":
        await admin_stats(update)
        return
    ...
    if data == "admin:adduser":
        ...
        await q.edit_message_text("➕ Username کاربر را ارسال کن (مثال: @Mmdm_mk):", reply_markup=InlineKeyboardMarkup([[btn("❌ لغو", "admin:menu")]]))
        return
    ...
    if cmd == "remsub":
        ...
        try:
            await context.bot.send_message(uid, get_text("subscription_expired"))
        except TelegramError:
            pass
    ...
    if cmd == "button":
        ...
        if action == "add":
            ...
            await q.edit_message_text("➕ فرمت: نام دکمه | action")
        elif action == "edit":
            context.user_data["state"] = "edit_button"
            await q.edit_message_text("✏️ فرمت: ID | label | action | enabled(0/1)")
        elif action == "delete":
...
async def expiry_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for uid in expired_ids:
        # Do not notify if the same user already received a replacement active subscription.
        ...
        if updated.rowcount != 1:
            continue
        try:
            await context.bot.send_message(uid, get_text("subscription_expired"), reply_markup=support_markup())
        except TelegramError:
            pass
